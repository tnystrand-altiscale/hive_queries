...
as select
    ...
    sum(case when (memory_job > 0.9*memory_max_capacity) then memory else 0 end) as max_mem_capacity_robbed_MBmin,
    sum(case when (vcores_job > 0.9*vcore_max_capacity ) then vcores else 0 end) as max_vcr_capacity_robbed_VCRmin,
